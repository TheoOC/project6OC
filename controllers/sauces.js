...
exports.createSauce = (req, res, next) => {
    //parse sauce because it is in form-data and not JSON because we have an image
    const sauceObject = JSON.parse(req.body.sauce);
    //delete _id if there is one. it s gonna get automatically generated by mongodb when the sauce id added to the database
...
exports.modifySauce = (req, res, next) => {
    ...
    //if there is an image delete the previous one
    if (req.file) {
        Sauce.findOne({ _id: req.params.id })
            .then(sauce => {
                const filename = sauce.imageUrl.split('/images/')[1];
                //update sauce in callback because if there is no callback there is an error
